...
function chaveSlot(quadraId, horaInicio) {
  return `${quadraId}|${horaInicio}`;
}

export default function Eventos({ quadras, modalidades }) {
  ...
  const carregar = useCallback(async () => {
    ...
    for (const q of quadras) {
      const { data: reservas } = await supabase
        .from('reservas')
        .select('id, hora_inicio, hora_fim, modalidade, evento_id, clientes(nome, telefone), eventos(nome_responsavel, observacao)')
        .eq('quadra_id', q.id)
        .eq('data', data)
        .neq('status_reserva', 'cancelada')
        .order('hora_inicio');

      const itensAvulsos = (reservas || []).map((r) => ({
        ...r,
        tipo: r.evento_id ? 'evento' : 'avulsa',
      }));
  ...
  function alternarSelecao(quadraId, horaInicio) {
    const chave = chaveSlot(quadraId, horaInicio);
  ...
  const listaSelecionados = Array.from(selecionados).map((chave) => {
    const [quadraId, horaInicio] = chave.split('|');
    const quadra = quadras.find((q) => q.id === quadraId);
    const slot = slots.find((s) => s.hora_inicio === horaInicio);
    return { quadraId, quadra, horaInicio, horaFim: slot?.hora_fim };
  });
  ...
  return (
    <div>
      ...
      <p className="text-areia-muted text-xs mb-6">Clique nos horários livres que o evento vai usar (pode marcar várias quadras e horários ao mesmo tempo).</p>

      {carregando && <p className="text-areia-muted mb-4">Carregando...</p>}

      {!carregando && slots.length === 0 && (
        <p className="text-aviso text-sm mb-4">Fechado nesse dia.</p>
      )}
                      ...
                      {item ? (
                        <>
                          ...
                          <div className="opacity-90 truncate">
                            {item.tipo === 'mensalista' ? 'Mensalista' : item.tipo === 'evento' ? 'Evento' : NOMES_MODALIDADE[item.modalidade]}
                          </div>
                        </>
                      ) : selecionado ? (
      ...
      {selecionados.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-night-panel border-t border-night-line p-4 flex items-center justify-between z-30">
          ...
          <div className="flex gap-3">
            <button onClick={() => setSelecionados(new Set())} className="text-areia-muted hover:text-areia text-sm px-3 py-2">
              Limpar seleção
            </button>
            <button
              onClick={() => setMostrarModal(true)}
              className="bg-coral hover:bg-coral-hover text-night font-semibold text-sm px-6 py-2 rounded-full"
            >
              Criar evento
            </button>
          </div>
...
function NovoEventoModal({ data, slotsSelecionados, modalidades, onFechar, onCriado }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <div className="bg-night-panel border border-night-line rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h3 className="font-display text-xl tracking-wide mb-1">NOVO EVENTO</h3>
        <p className="text-areia-muted text-xs mb-4">
          {slotsSelecionados.length} horário{slotsSelecionados.length > 1 ? 's' : ''} selecionado{slotsSelecionados.length > 1 ? 's' : ''}:
        </p>
        <div className="bg-night rounded-lg p-3 mb-4 text-xs text-areia-muted space-y-1 max-h-24 overflow-y-auto">
          {slotsSelecionados.map((s) => (
            <div key={`${s.quadraId}-${s.horaInicio}`}>{s.quadra?.nome} · {s.horaInicio}–{s.horaFim}</div>
          ))}
        </div>
        ...
        <label className="block mb-4">
          ...
          <textarea
            ...
            onChange={(e) => setObservacao(e.target.value)}
            rows={4}
            placeholder="Ex: aniversário de 15 anos, torneio interno, festa de confraternização..."
            className="bg-night border border-night-line rounded-lg px-3 py-2 text-areia w-full resize-none"
          />
        </label>
