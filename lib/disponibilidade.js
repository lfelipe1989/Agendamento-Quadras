...
// Confere se um horário (dia da semana + início/fim) está dentro do horário de
// funcionamento configurado pra esse dia. Usado ao criar/editar mensalista, já
// que não tem uma data específica pra rodar buscarHorariosDisponiveis normalmente.
export async function dentroDoHorarioFuncionamento(diaSemana, horaInicio, horaFim) {
  const { data: horarioDia } = await supabase
    .from('horarios_funcionamento')
    .select('*')
    .eq('dia_semana', diaSemana)
  ...
  const abertura = horarioDia.hora_abertura.slice(0, 5);
  const fechamento = horarioDia.hora_fechamento.slice(0, 5);

  return horaInicio >= abertura && horaFim <= fechamento;
}
